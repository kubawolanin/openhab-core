...
/**
 * The dashboard tile and resource registering for HomeBuilder
 *
...
public class HomeBuilderDashboardTile implements DashboardTile {
    ...
    protected void activate() {
        try {
            ...
            logger.info("Started HomeBuilder at {}", HOMEBUILDER_ALIAS);
        } catch (NamespaceException e) {
            logger.error("Error during HomeBuilder startup: {}", e.getMessage());
        }
    }

    protected void deactivate() {
        httpService.unregister(HOMEBUILDER_ALIAS);
        logger.info("Stopped HomeBuilder");
    }
